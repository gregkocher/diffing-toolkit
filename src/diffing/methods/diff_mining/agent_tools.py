...
def _method_dir_for_extraction(
    *,
    ...
    assert (
        variant_dir.exists() and variant_dir.is_dir()
...
def get_overview(
    ...
    assert hasattr(
        method, "_get_run_folder_name"
    ), "DiffMiningMethod must define _get_run_folder_name"
    ...
    assert hasattr(
        method, "_ordering_dir_name"
    ), "DiffMiningMethod must define _ordering_dir_name"
