...
from .core_analysis import (
    CoreAnalysisResult,
    compute_stats_from_logits as _compute_stats_from_logits,
)
...
class DiffMiningMethod(DiffingMethod):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        assert (
            token_ordering_cfg is not None
        ), "diff_mining config must define token_ordering"
        ...
        assert len(set(self.ordering_methods)) == len(
            self.ordering_methods
        ...
        if "nmf" in self.ordering_methods:
            ...
            ), "token_ordering.nmf must be set when nmf is enabled"
            assert bool(
                getattr(self.nmf_cfg, "enabled", True)
            ...
            assert hasattr(
                self.nmf_cfg, "top_n_tokens_per_topic"
            ...
            assert (
                self.nmf_top_n_tokens_per_topic > 0
        ...
        if self.logit_extraction_method == "logits":
            ...
            position_batch_size = int(
                getattr(
                    logit_extraction_cfg.patchscope_lens, "position_batch_size", 256
                )
            )
            patch_prompt = str(
                getattr(
            ...
            index_to_patch = int(
                getattr(logit_extraction_cfg.patchscope_lens, "index_to_patch", -1)
            )
    ...
    def _get_run_folder_name(self) -> str:
        ...
        assert hasattr(
            self.cfg, "seed"
        ), "cfg.seed must be set for deterministic outputs"
        seed = self.cfg.seed
    ...
    def _get_enabled_ordering_types(self) -> List[TokenOrderingType]:
        ...
        for method in self.ordering_methods:
            if method == "top_k_occurring":
                ...
                assert (
                    self.nmf_cfg is not None
                ), "nmf_cfg must be set when 'nmf' is enabled"
                assert bool(
                    getattr(self.nmf_cfg, "enabled", True)
                ...
                nmf_config = NmfOrderingConfig(
                    ...
                    orthogonal_weight=float(
                        getattr(self.nmf_cfg, "orthogonal_weight", 1.0)
                    ),
    ...
    def _ordering_dir_name(self, ordering_type_id: str) -> str:
        ...
        if ordering_type_id == "top_k_occurring":
            assert hasattr(
                token_ordering_cfg, "top_k_occurring"
        ...
        if ordering_type_id == "fraction_positive_diff":
            assert hasattr(
                token_ordering_cfg, "fraction_positive_diff"
        ...
        if ordering_type_id == "nmf":
            ...
            assert (
                nmf_cfg is not None
            ), "diff_mining config must define token_ordering.nmf"
            assert bool(
                getattr(nmf_cfg, "enabled", True)
            ), "token_ordering.nmf.enabled must be true"
    ...
    def _run_ordering_types_and_write(
        ...
        for ordering_type in ordering_types:
            ...
            if not result.orderings:
                self.logger.warning(
                    f"No orderings produced for {ordering_type.ordering_type_id}"
                )
                continue

            # Write ordering type directory
            ordering_type_dir = run_dir / self._ordering_dir_name(
                result.ordering_type_id
            )
    ...
    def setup_models(self):
        ...
        self.logger.info(
            f"Loading finetuned model: {self.finetuned_model_cfg.model_id}"
        )
        _ = self.finetuned_model
        self.logger.info(
            f"✓ Finetuned model loaded: {self.finetuned_model_cfg.model_id}"
        )
    ...
    def compute_sequence_likelihood_ratios(
        ...
        if dataset_name in self._log_probs:
            ...
            if not input_ids_path.exists():
                self.logger.warning(
                    f"Input IDs not found for {dataset_name}. Please re-run preprocessing."
                )
                return None

            # Load tensors
            base_log_probs = torch.load(
                base_log_probs_path, map_location="cpu"
            )  # [num_samples, seq_len-1]
            ft_log_probs = torch.load(
                ft_log_probs_path, map_location="cpu"
            )  # [num_samples, seq_len-1]
            input_ids = torch.load(
                input_ids_path, map_location="cpu"
            )  # [num_samples, seq_len] or sliced
            attention_mask = torch.load(
                mask_path, map_location="cpu"
            )  # [num_samples, seq_len]
        ...
        for sample_idx in range(num_samples):
            ...
            for start_pos in range(0, valid_len - window_size + 1, step):
                ...
                window_base_logprob = (
                    base_log_probs[sample_idx, start_pos:end_pos].sum().item()
                )
                window_ft_logprob = (
                    ft_log_probs[sample_idx, start_pos:end_pos].sum().item()
                )
        ...
        self.logger.info("-" * 80)
        self.logger.info(
            f"Showing top {min(top_k_print, len(chunks_sorted))} of {len(chunks_sorted)} chunks"
        )
        self.logger.info("")

        # Save to JSON
        output_path = (
            self.analysis_dir / f"{dataset_name}_sequence_likelihood_ratios.json"
        )
    ...
    def _build_occurrence_results_for_token_ids(
            ...
            all_tokens.append(
                {
                    ...
                    "positive_occurrence_rate": (
                        (pos_count / total_positions) * 100
                        if total_positions > 0
                        else 0.0
                    ),
                    "negative_occurrence_rate": (
                        (neg_count / total_positions) * 100
                        if total_positions > 0
                        else 0.0
                    ),
                }
        ...
        num_tokens_to_save = max(
            int(self.method_cfg.visualization.num_tokens_to_plot), int(top_k)
        )
        ...
        top_k_pos_values, top_k_pos_indices = torch.topk(
            pos_rates, k=k_pos, largest=True
        )
        top_k_neg_values, top_k_neg_indices = torch.topk(
            neg_rates, k=k_neg, largest=True
        )
    ...
    def run_token_relevance(
        ...
        ), "run_dir must be set before running token relevance"
        ...
        if has_training_dataset:
            ...
            logger.info(
                f"Computing frequent tokens from training dataset: {training_dataset_id}"
            )
            ...
            frequent_tokens = []
            logger.info(
                "No training dataset available, using empty frequent tokens list"
            )
        ...
        for dataset_name, ordering_type_results in all_ordering_results.items():
            for ordering_type_id, type_result in ordering_type_results.items():
                ...
                dataset_dir = (
                    self.run_dir
                    / self._ordering_dir_name(ordering_type_id)
                    / dataset_name
                )
                ...
                for ordering in type_result.orderings:
                    ...
                    if (not overwrite) and eval_path.exists():
                        logger.info(
                            f"Eval exists for {ordering_type_id}/{dataset_name}/{ordering_id}, skipping"
                        )
                        continue
                    ...
                    logger.info(
                        f"Grading {k_tokens} tokens for {ordering_type_id}/{dataset_name}/{display_label}"
                    )
                    num_batches = (
                        k_tokens + token_relevance_batch_size - 1
                    ) // token_relevance_batch_size
                    logger.info(
                        f"Token relevance: {num_batches} batch(es) of size {token_relevance_batch_size}"
                    )

                    trivial_hits = sum(
                        1 for t in candidate_tokens if t in frequent_tokens
                    )
                    trivial_percentage = (
                        trivial_hits / float(len(candidate_tokens))
                        if candidate_tokens
                        else 0.0
                    )
                    ...
                    if include_translation:
                        ...
                        async def _run_all_batches_with_translation() -> (
                            List[
                        ...
                        async def _run_all_batches() -> (
                            List[Tuple[List[str], List[List[str]], List[str]]]
                        ):
                            tasks = [
                                grader.grade_async(
                                    description=description,
                    ...
                    logger.info(
                        f"  Relevance: {relevant_fraction*100:.1f}%, Weighted: {weighted_percentage*100:.1f}%"
                    )
    ...
    def run(self) -> None:
        ...
        assert hasattr(
            self.method_cfg, "overwrite"
        ), "diff_mining config must define overwrite"
        ...
        for idx, dataset_cfg in enumerate(self.datasets, 1):
            ...
            if use_in_memory:
                ...
                if dataset_cfg.name not in self._logit_diffs:
                    self.logger.warning(
                        f"No in-memory diff for {dataset_cfg.name}, skipping"
                    )
                    continue
        ...
        if (
            hasattr(self.method_cfg, "token_relevance")
    ...
    def preprocess(self, delete_raw: bool = True) -> None:
        ...
        if self._in_memory:
            self.logger.info(
                "In-memory mode: computing diffs inline during finetuned inference."
            )
            logit_diffs, log_probs, attention_masks, input_ids_out = (
                infer_finetuned_and_compute_diffs_in_memory(
                    self.datasets,
                    dataset_inputs,
        ...
        if self._in_memory:
            self.logger.info(
                "In-memory mode: diffs already computed. Skipping disk-based Phase 3."
            )
        ...
        for base_model_dir in results_base.iterdir():
            ...
            for organism_dir in base_model_dir.iterdir():
                ...
                for method_dir in organism_dir.iterdir():
                    if method_dir.is_dir() and method_dir.name.startswith(
                        "diff_mining"
                    ):
                        run_folders = sorted(
                            [
                                ...
                                if d.is_dir()
                                and re.match(r"seed[0-9]+_top[0-9]+$", d.name)
                            ],
