...
class EvaluationPipeline(Pipeline):
    ...
    def run_agent(
        ...
        async def grade_all():
            tasks = [
                grade_and_save_async(
                    self.cfg, description, save_dir=out_dir, run_id=grader_run_idx
                )
                for grader_run_idx in range(grader_num_repeat)
            ]
        ...
        agent_scores = []
        for grader_run_idx, (agent_score, _agent_text) in enumerate(results):
            logger.info(
                f"Graded agent (run {run_idx}, grader run {grader_run_idx}) description with score={agent_score}"
            )
            agent_scores.append(agent_score)

        agent_score = float(sum(agent_scores)) / float(len(agent_scores))
        _agent_text = results[-1][1] if results else ""
        # Save config
    ...
    def run(self) -> None:
        ...
        grader_num_repeat = int(self.evaluation_cfg.grader.num_repeat)
        assert num_repeat >= 1
        assert grader_num_repeat >= 1
        ...
        for model_interaction_budget in agent_cfg.budgets.model_interactions:
            ...
            for run_idx in range(num_repeat):
                ...
                agent_score, _agent_text, description = self.run_agent(
                    ...
                    overwrite,
                    agent.name + "_" + name,
                    hints=agent_cfg.hints,
                    grader_num_repeat=grader_num_repeat,
                )
