...
from diffing.methods.diff_mining.logit_extraction import DirectLogitsExtractor, RecurrenceLogitsExtractor
...
for arm in ("base", "target"):
    ...
    hook.remove()
    observed=[]
    handle=underlying.model.register_forward_hook(lambda m,a,o: observed.extend(o[1]))
    with torch.no_grad(): native(**inputs)
    handle.remove()
    for recurrence in range(4):
        with torch.no_grad():
            projected=underlying.lm_head(observed[recurrence]).float().cpu()
            selected=RecurrenceLogitsExtractor(recurrence_idx=recurrence).extract_logits(wrapped,**inputs).float().cpu()
        assert torch.equal(projected,selected), f"recurrence {recurrence} mismatch"
        if recurrence==3: assert torch.equal(selected,actual)
    assert visits==[0,1,2,3],visits
...
Path("/workspace/methods_v2/parity.json").write_text(json.dumps(records, indent=2)+"\n")
