...
for i, prompt in enumerate(prompts[:a.max_prompts]):
    ...
    if path.exists():
        ...
        lens = fit(model, [prompt], source_layers=[0, 1, 2], target_layer=3,
            ...
            checkpoint_path=str(out/f'prompt_{i:03d}_checkpoint.pt'))
        lens.save(str(path), dtype=torch.float32)
    assert all(torch.isfinite(j).all() for j in lens.jacobians.values())
    summands.append(lens)
    merged = JacobianLens.merge(summands)
    n = len(summands)
    merged.save(str(out/f'lens_n{n}.pt'), dtype=torch.float32)
    ...
    previous = merged
model.close()
(out/'COMPLETE.json').write_text(json.dumps({'n_prompts': len(summands), 'settings': vars(a),
